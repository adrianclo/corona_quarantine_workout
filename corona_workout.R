...
# extra settings
jumping_rope <- T # replace jumping rope?
jumping_jack <- T # replace jumping jack?
burpees <- T      # replace burpees?
extra_hard <- F   # >25 are counted backwards, and workout is finished once landing exactly on 25
level <- c()      # easy - medium - hard
...
game <- matrix(c(
  ...
  "8",  "horizontal mountain climbers",  "1", "30", "s", NA,
...
colnames(game) <- c("square", "instruction", "set", "time", "unit", "extra")
game <- as_tibble(game)

if(jumping_rope) game[10,2] <- "stationary running"
if(jumping_jack) game[5,2] <- "standing mountain climbers"
if(burpees) game[20,2] <- "push ups"
# game
